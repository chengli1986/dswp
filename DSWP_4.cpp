...
void DSWP::preLoopSplit(Loop *L) {
  ...
  StructType *outStructTy = StructType::create(*context, liveoutTypes, "outstruct_" + itoa(loopCounter) + "_ty");
  ...
  argStructTy = StructType::create(*context, argTypes, "argstruct_" + itoa(loopCounter) + "_ty");
  ...
  for (unsigned int i = 0; i < livein.size(); i++) {
    ...
    // chwlo: this is actually ok... why?
    GetElementPtrInst *ele_addr = GetElementPtrInst::CreateInBounds(argStruct, gep_args, livein[i]->getName() + "_argptr", brInst);
  ...
  CastInst *argStruct_voidPtr = CastInst::CreatePointerCast(argStruct, int8_ptr_t, "argstruct_" + itoa(loopCounter) + "_cast", brInst);
  ...
  if (!liveout.empty()) {
    ...
    GetElementPtrInst *out_addr = GetElementPtrInst::CreateInBounds(argStruct, gep_args, "load_outs", brInst);
    ...
    for (unsigned int i = 0; i < liveout.size(); i++) {
      ...
      gep_args.push_back(ConstantInt::get(int64_ty, 0));
      gep_args.push_back(ConstantInt::get(int32_ty, i));
      GetElementPtrInst *ele_addr = GetElementPtrInst::CreateInBounds(out_addr, gep_args, liveout[i]->getName() + "_ptr", brInst);

      // load the out value
      LoadInst *outVal = new LoadInst(ele_addr, liveout[i]->getName() + "_load", brInst);
...
void DSWP::loopSplit(Loop *L) {
  ...
  for (int i = 0; i < MAX_THREAD; i++) {
    ...
    for (vector<int>::iterator ii = part[i].begin(), ie = part[i].end(); ii != ie; ++ii) {
      int scc = *ii;
      for (vector<Instruction *>::iterator iii = InstInSCC[scc].begin(), iie = InstInSCC[scc].end(); iii != iie; ++iii) {
    ...
    if (relbb.empty()) {
      ...
      BasicBlock *newBody = BasicBlock::Create(*context, "do-nothing", curFunc);
      ReturnInst *newRet = ReturnInst::Create(*context, Constant::getNullValue(Type::getInt8PtrTy(*context)), newBody);
      return;
    }
    ...
    for (set<BasicBlock *>::iterator bi = relbb.begin(), be = relbb.end(); bi != be; ++bi) {
      BasicBlock *BB = *bi;
      BBMap[BB] = BasicBlock::Create(*context, BB->getName().str() + "_" + itoa(i), curFunc, newExit);
    }
    ...
    // return null
    ReturnInst *newRet = ReturnInst::Create(*context, Constant::getNullValue(Type::getInt8PtrTy(*context)), newExit);
    ...
    for (set<BasicBlock *>::iterator bi = relbb.begin(), be = relbb.end(); bi != be; bi++) {
      ...
      for (BasicBlock::iterator ii = BB->begin(), ie = BB->end(); ii != ie; ii++) {
        ...
        for (to_point_t::iterator pi = instructions_to_point.begin(), pe = instructions_to_point.end(); pi != pe; ++pi) {
          Instruction *p = *pi;
    ...
    Argument *args = arglist.begin(); //the function only have one argument
    ...
    BitCastInst *castArgs = new BitCastInst(args, PointerType::get(argStructTy, 0), "args");
    ...
    for (unsigned int j = 0, je = livein.size(); j < je; j++) {
      ...
      //GetElementPtrInst* ele_addr = GetElementPtrInst::Create(castArgs, gep_args, livein[j]->getName() + "_arg", newToHeader);
      // chwlo DANGEROUS not sure if it's correct or not
      //GetElementPtrInst* ele_addr = GetElementPtrInst::Create(castArgs->getType(), nullptr, gep_args, livein[j]->getName() + "_arg", newToHeader);
      GetElementPtrInst* ele_addr = GetElementPtrInst::CreateInBounds(castArgs, gep_args, livein[j]->getName() + "_arg", newToHeader);
     ...
     */
    for (inst_iterator ii = inst_begin(curFunc), ie = inst_end(curFunc); ii != ie; ++ii) {
      Instruction *inst = &(*ii);
    ...
    gep_args.push_back(ConstantInt::get(int64_ty, 0));
    gep_args.push_back(ConstantInt::get(int32_ty, livein.size()));
    GetElementPtrInst *out_addr = GetElementPtrInst::CreateInBounds(castArgs, gep_args, "load_outs", newRet);
    ...
    for (unsigned int j = 0; j < liveout.size(); j++) {
      if (getInstAssigned(liveout[j]) == i) {
        ...
        GetElementPtrInst *ele_addr = GetElementPtrInst::CreateInBounds(out_addr, gep_args, liveout[j]->getName() + "_outptr", newRet);

        // save it
        StoreInst *store = new StoreInst(instMap[i][liveout[j]], ele_addr, newRet);
      }
    }
  }
}
...
void DSWP::getLiveinfo(Loop *L) {
  ...
  // Find variables defined in the loop, and those that are used outside the loop.
  for (Loop::block_iterator bi = L->getBlocks().begin(), be = L->getBlocks().end(); bi != be; ++bi) {
    BasicBlock *BB = *bi;
    for (BasicBlock::iterator ii = BB->begin(), ie = BB->end(); ii != ie; ++ii) {
      Instruction *inst = &(*ii);
      ...
      for (Instruction::user_iterator ui = inst->user_begin(), ue = inst->user_end(); ui != ue; ++ui) {
        // chwlo DANGEROUS!
        //User *use = *ui;
        User *use = *ui;
