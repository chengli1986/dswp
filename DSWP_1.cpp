...
void DSWP::dfsVisit(BasicBlock *BB, std::set<BasicBlock *> &vis, std::vector<BasicBlock *> &ord, Loop *L) {
...
void DSWP::buildPDG(Loop *L) {
  ...
  for (Loop::block_iterator bi = L->getBlocks().begin(); bi != L->getBlocks().end(); bi++) {
      ...
      for (BasicBlock::iterator ii = BB->begin(); ii != BB->end(); ii++) {
          ...
          cout << endl << "\t";
          inst->print(outstream);
          cout << endl << "\t" << "isClobber: " << mdr.isClobber()
                   << "\t" << "isDef: " << mdr.isDef()
          ...
          if (mdr.isNonLocal()) {
              if (CallInst *call = dyn_cast<CallInst>(inst)) {
                  const MemoryDependenceAnalysis::NonLocalDepInfo &rs = mda.getNonLocalCallDependency(call);
                  for (MemoryDependenceAnalysis::NonLocalDepInfo::const_iterator ri = rs.begin(), re = rs.end(); ri != re; ++ri) {
                  ...
                  res_t res;
                  //mda.getNonLocalPointerDependency(MemLoc, is_load, inst->getParent(), res);
                  mda.getNonLocalPointerDependency(inst, res);

                  for (res_t::iterator ri = res.begin(), re = res.end(); ri != re; ++ri) {
  ...
  Constant *cfunc = newmodule.getOrInsertFunction("dummyloopunroll", Type::getVoidTy(getGlobalContext()), int_arg, NULL);
  cout<<">>Function created!"<<endl;
  Function *ctrlfunc = cast<Function>(cfunc);

  /*
   * Function *ctrlfunc = Function::Create(functype, Function::ExternalLinkage, "dummyloopunroll", module);
   */
  ...
  for (std::vector<BasicBlock *>::iterator it = bb_ordered.begin(); it !=bb_ordered.end(); ++it) {
      cout<<(*it)->getName().str()<<", ";
  }
  cout<<endl;
  ...
  if (!bb_ordered.empty()) {
      ...
      do {
          ...
          BasicBlock *newbb = BasicBlock::Create(ctxt, n1, ctrlfunc, 0);

          //Dummy block for second iteration
          BasicBlock *newbb2 = BasicBlock::Create(ctxt, n2, ctrlfunc, 0);
  ...
  BasicBlock *dummyexitblock = BasicBlock::Create(ctxt, "exitblock", ctrlfunc, 0);
  ReturnInst::Create(ctxt, 0, dummyexitblock);

  cout<<">>Printing out names of dummy blocks inside our fake function"<<endl;
  for (Function::iterator FI = ctrlfunc->begin(), FE = ctrlfunc->end(); FI != FE; ++FI) {
      cout<<(*FI).getName().str()<<", ";
  }
  ...
  for (SmallVector<BasicBlock *, 10>::iterator it = bb_exits.begin(), ie = bb_exits.end(); it != ie; ++it) {
      BasicBlock *bbend = *it;
      for (pred_iterator pi = pred_begin(bbend), pe = pred_end(bbend); pi != pe; ++pi) {
  ...
  if (!bb_ordered.empty()) {
      ...
      do {
          ...
          if (returnblocks.find(*it) != returnblocks.end()) {
              SI1 = builder1.CreateSwitch(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0), dummyexitblock, nsucc+1);
              SI2 = builder2.CreateSwitch(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0), dummyexitblock, nsucc+1);
              SI1->addCase(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0), dummyexitblock);
              SI2->addCase(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0), dummyexitblock);
              swcreated1 = true;
              swcreated2 = true;
          }
          ...
          for (unsigned i = 0; i < nsucc; i++) {
              ...
              if (L->contains(bsucc)) { //successor is still inside loop?
                  ...
                  if (!swcreated1) { //need to create switch instruction for top half
                      SI1 = builder1.CreateSwitch(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0), destblock1, nsucc);
                      swcreated1 = true;
                  }

                  //Add a case to the switch instruction for the top half
                  SI1->addCase(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), i+1), destblock1);

                  //Now, deal with instruction for bottom half
                  if (!swcreated2) { //need to create switch instr. for bottom half
                      SI2 = builder2.CreateSwitch(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0), destblock2, nsucc);
                      swcreated2 = true;
                  }

                  //Add a case to the switch instruction for the bottom half
                  SI2->addCase(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), i+1), destblock2);
              }
          }
      } while (it != bb_ordered.begin());
  }
  ...
  for (std::vector<BasicBlock *>::iterator it = dummylist.begin(); it != dummylist.end(); ++it) {
      TerminatorInst *tinst = (*it)->getTerminator();
      unsigned nsucc = tinst->getNumSuccessors();
      ...
      for (unsigned i = 0; i < nsucc; i++) {
          ...
          while (succnode != dn && depblock != dummyexitblock) {
              ...
              for (BasicBlock::iterator bi = realdepblock->begin(), be = realdepblock->end(); bi != be; ++bi) {
                  Instruction *inst = &(*bi);
